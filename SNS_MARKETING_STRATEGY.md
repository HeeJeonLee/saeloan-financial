# 새론금융대부중개 SNS 영업 자동화 전략
### AI Agency 방식 | 2026-05-21 최종 수정 | 운영: 비공개 (대외 명의: 김덕진 대표)
### ⚠️ 이 파일은 SNS_SALES_SYSTEM.md v4.0 (5-Track 최신 전략)과 함께 사용할 것

---

## 0. ⚠️ 운영 구조 핵심 원칙 (절대 불변)

### 필수 전제 조건

> **이희전 팀장은 우리은행 소속 대출상담사(40명 관리 팀장)이므로,**
> **대부중개업과의 겸직이 불가능합니다.**
> **따라서 모든 SNS·광고·홈페이지는 반드시 김덕진 대표 명의로만 운영해야 합니다.**

| 항목 | 내용 |
|---|---|
| 홈페이지 소유 | 김덕진 (도메인, 사업자 명의) |
| 실제 운영·관리 | 비공개 (대외 노출 절대 금지) |
| SNS 계정 개설 | 이희전 계정으로 개설하되 **김덕진 명의·얼굴·이름으로만 운영** |
| SNS 대외 명의 | **김덕진 대표만 노출** (이희전 이름·직책 SNS 노출 금지) |
| 대표전화 | 010-5927-9205 (김덕진 명의 / 이희전 실수신) |
| 공식 전화 (홈페이지) | 1555-2137 |
| 상담신청 폼·이메일 | 홈페이지 내부 처리 (SNS 직접 노출 금지) |

### 운영 구조 배경
- 김덕진 대표는 SNS 운영이 어려워 이희전에게 위임함 (승인 완료)
- 이희전은 은행 소속 신분 노출 시 겸직 문제 발생 → SNS상 존재 완전 비공개
- 모든 콘텐츠, 계정 소개, 상담, 광고는 **"김덕진 대표"가 직접 운영하는 것처럼 제작**

### 절대 금지 항목
- SNS 어디에도 이희전 이름·소속(우리은행) 노출 금지
- "운영자 따로 있음" 암시 표현 금지
- 얼굴·목소리 출연 시 반드시 김덕진 대표 본인이거나 등장 인물 생략 처리

**원칙:** SNS 모든 CTA는 `saeloan.co.kr` 링크로 통일 → 홈페이지에서만 상담 처리

---

## 1. 법적 준수 사항 (필수)

### 대부업법 및 금융광고 규정
- **금리 표시 의무:** 광고 시 최고 금리(현행 연 20%) 반드시 명시
- **등록번호 표시:** 대부중개업 등록번호 모든 광고물에 기재
- **경고 문구 의무:** "과도한 빚은 당신에게 큰 불행을 안겨줄 수 있습니다" 문구 포함
- **허위·과장 광고 금지:** "100% 승인", "무조건 대출" 등 표현 절대 금지
- **미등록 업체 사칭 금지:** 저축은행/캐피탈 직접 취급 오해 유발 표현 금지
- **개인정보:** SNS DM으로 개인정보(주민번호, 계좌번호) 수집 금지 → 홈페이지 폼으로만

### SNS별 금융광고 정책
- **인스타그램/페이스북:** 금융상품 광고 사전 승인 필요 (Meta 금융서비스 정책)
- **유튜브:** 금융 콘텐츠 수익화 제한 있으나 정보성 콘텐츠는 허용
- **카카오채널:** 대부업 광고 허용 (등록증 제출 필요)
- **네이버 블로그:** 정보성 포스팅은 허용, 직접 광고는 검색광고 정책 준수

---

## 2. 글로벌 최고 수준 SNS 영업 전략 분석

### 2-1. 미국 금융 마케팅 벤치마크 (Dave Ramsey / SoFi / LendingTree)
- **교육 콘텐츠 우선 전략:** "당신의 신용점수를 올리는 7가지 방법" → 신뢰 구축 후 상담 유도
- **스토리텔링:** 실제 고객 사례(익명) 기반 Before/After 콘텐츠
- **Retargeting:** 홈페이지 방문자에게 SNS 리타겟 광고
- **유료 리드 생성:** 무료 한도조회 → 이메일/연락처 취득 → 자동 팔로업

### 2-2. 일본 소비자금융 SNS 전략 (아코무, SMB C파이낸스)
- **라인(LINE) 공식계정 중심:** 1:1 상담 채널로 활용
- **단계별 안내:** 무료조회 → 간단서류 → 당일승인 프로세스 시각화
- **유튜브 쇼츠:** 60초 이내 "이것만 알면 대출 된다" 형식

### 2-3. 국내 최고 사례 분석
- **OK저축은행:** 유튜브 "OK금융그룹" 채널 - 재무교육 + 상품안내 혼합
- **핀다(Finda):** 인스타그램 카드뉴스 - 복잡한 금융정보를 이미지로 단순화
- **뱅크샐러드:** 개인화 데이터 기반 콘텐츠 → 신뢰도 극대화

---

## 3. 새론금융 플랫폼별 실행 전략

### 3-1. 카카오채널 (최우선 채널)
**이유:** 한국 40~60대 주요 고객층의 메인 플랫폼

**설정:**
```
채널명: 새론금융대부중개 (김덕진 대표)
카테고리: 금융 > 대출
프로필: 대표 이미지 + 등록번호 표시
자동응답: 24시간 챗봇 설정
```

**자동화 챗봇 시나리오:**
```
[방문자 진입]
    ↓
"안녕하세요! 새론금융대부중개입니다 😊
무엇을 도와드릴까요?"
    ↓
[버튼 1] 무료 한도조회  [버튼 2] 금리 안내  [버튼 3] 상담 신청
    ↓
[무료 한도조회 선택 시]
"신용점수 영향 없이 예상 한도를 확인하세요
→ saeloan.co.kr 에서 무료조회 가능합니다"
    ↓
[상담 신청 선택 시]
"전담 상담사가 연락드립니다.
이름과 연락처를 남겨주시면 빠르게 안내해 드리겠습니다.
(개인정보는 안전하게 보호됩니다)"
```

**주간 발송 콘텐츠 (자동화):**
- 월: 이번 주 저축은행 금리 동향
- 수: 대출 상식 카드뉴스 1장
- 금: 주말 상담 가능 안내

---

### 3-2. 인스타그램
**계정:** @saeloan_financial (대외 명의: 김덕진 대표)

> ⚠️ 계정 소개란, 게시물, 스토리 어디에도 이희전 이름 노출 금지
> 프로필 소개: "새론금융대부중개 김덕진 대표 | 대출 정보 채널"

**콘텐츠 유형 (7:2:1 법칙):**
- 70% 교육·정보 콘텐츠 (신용관리, 대출 상식) — 텍스트·이미지 중심, 얼굴 출연 없이 제작
- 20% 상품 안내 (파트너사 금리, 한도)
- 10% 신뢰 구축 (등록증 이미지, 등록번호 명시)

**게시물 구조 (카드뉴스 형식):**
```
슬라이드 1: 훅(Hook) - "신용불량도 대출 받을 수 있을까요?"
슬라이드 2-5: 핵심 내용 (짧고 굵게)
슬라이드 6: CTA - "무료조회: saeloan.co.kr"
슬라이드 7: 법적 고지 (금리 범위, 등록번호, 경고문구)
```

**해시태그 전략:**
```
#대출 #신용대출 #저축은행대출 #비대면대출 #당일대출
#대출상담 #소액대출 #생활비대출 #긴급대출 #새론금융
#대부중개 #한도조회 #무료조회 #금융정보
```

**릴스(Reels) 60초 스크립트 예시:**
```
[0-5초] "혹시 이렇게 말 들어보셨나요? '당신은 대출이 안 됩니다'"
[5-20초] "대출이 거절되는 3가지 이유 (신용점수/소득증빙/부채비율)"
[20-45초] "새론금융에서는 이렇게 해결합니다 (파트너 6개사 비교)"
[45-55초] "지금 무료로 한도 확인하세요"
[55-60초] "saeloan.co.kr / ☎ 1555-2137"
[화면 하단 자막] "연 6.9~19.9% / 등록번호 OOO / 과도한 빚은..."
```

---

### 3-3. 유튜브 쇼츠 & 채널
**채널명:** 새론금융 김덕진 | 대출 바로알기

> ⚠️ 채널명·소개·영상 내 이희전 이름 노출 금지
> 얼굴 출연이 필요한 경우 → 김덕진 대표 직접 출연하거나, 자막+화면 구성만으로 제작
> 목소리 지문(TTS 또는 김덕진 대표 직접 녹음)으로 처리 권장

**쇼츠 콘텐츠 시리즈:**
```
시리즈 1: "대출 ○○ 했다가 망했어요" - 실수 유형 정리
시리즈 2: "이 조건이면 얼마나 받을까?" - 케이스별 시뮬레이션
시리즈 3: "저축은행 vs 카드론 vs 대부 차이점"
시리즈 4: "상담사가 알려주는 진짜 꿀팁"
```

**업로드 일정:** 주 2회 (화, 목 오전 9시)

---

### 3-4. 네이버 블로그
**용도:** SEO + 정보성 콘텐츠 → 검색 유입

**주요 키워드 포스팅:**
```
- "아파트담보대출 은행 거절 후 방법 (2026년 최신)"
- "후순위 담보대출이란? 조건과 절차 완전정리"
- "대부중개업체 이용 방법과 주의사항"
- "DSR 초과 시 아파트담보 대출 받는 법"
- "자영업자 아파트 담보로 운영자금 조달하는 법"
```

**포스팅 구조:**
1. 제목 (검색키워드 포함)
2. 요약 (3줄)
3. 본문 (800~1,500자, 이미지 5장 이상)
4. 마무리 CTA: "새론금융 무료 한도조회 바로가기 → saeloan.co.kr"
5. 면책/법적 고지 (금리범위, 등록번호)

---

## 4. AI 자동화 시스템 구성

### 4-1. 콘텐츠 자동 생성 파이프라인

```
[매주 월요일 자동 실행]
1. Google Gemini API → 주간 금융이슈 기반 콘텐츠 초안 생성 (Google Apps Script)
2. Canva → 카드뉴스 이미지 제작 (브랜드 템플릿 적용)
3. Buffer/직접 업로드 → 각 플랫폼에 예약 발행
4. 이희전 최종 검토 → 승인 후 발행
```

### 4-2. 추천 자동화 툴 스택

| 역할 | 도구 | 월 비용 |
|---|---|---|
| 콘텐츠 생성 | **Google Gemini API** (gemini-1.5-flash) | **무료** |
| 자동화 엔진 | **Google Apps Script** | **무료** |
| 이미지 생성 | Canva (무료) / Canva Pro (월 15건 초과 시) | 무료~17,000원 |
| SNS 예약발행 | Buffer (무료 3채널) | 무료 |
| TTS 음성 | ElevenLabs (월 15건 초과 시) | 무료~6,000원 |
| CRM / 리드관리 | Google Sheets (연동 완료) | 무료 |
| 이메일 자동화 | Gmail + Google Apps Script | 무료 |
| **합계 (Phase 1)** | | **월 0원** |

### 4-3. 리드(상담신청) 자동 처리 흐름

```
SNS 게시물 → 홈페이지 링크 클릭
    ↓
saeloan.co.kr 상담신청 폼 작성
    ↓
이희전 이메일로 자동 수신 (현재 API 구현 완료)
    ↓
Google Sheets 자동 기록
    ↓
30분 이내 답장 자동 발송 (Google AppScript)
    ↓
전담 상담사(이희전) 전화 상담 → 파트너사 연결
```

---

## 5. 주간 콘텐츠 캘린더 (자동화 기준)

| 요일 | 플랫폼 | 콘텐츠 유형 |
|---|---|---|
| 월 | 카카오채널 | 이번 주 저축은행 금리 동향 |
| 월 | 네이버 블로그 | 정보성 포스팅 1건 |
| 화 | 인스타그램 | 카드뉴스 (교육 콘텐츠) |
| 화 | 유튜브 쇼츠 | 60초 영상 |
| 수 | 인스타그램 스토리 | 간단 퀴즈 or 투표 |
| 목 | 유튜브 쇼츠 | 60초 영상 |
| 목 | 카카오채널 | 상담 가능 안내 |
| 금 | 인스타그램 | 카드뉴스 (상품 안내) |
| 토 | 인스타그램 스토리 | 주말 긴급상담 가능 안내 |

---

## 6. 콘텐츠 샘플

### 샘플 1: 인스타그램 카드뉴스 (교육형)
> 모든 샘플: 발신자 = 김덕진 대표. 이희전 언급 없음.
```
[슬라이드 1] 📊 신용점수 이것만 알면 10점 오릅니다

[슬라이드 2] ✅ 체크카드보다 신용카드를 쓰세요
              (단, 30% 이하 사용)

[슬라이드 3] ✅ 대출을 여러 곳에 동시 신청하지 마세요
              (단기간 다중조회 = 감점)

[슬라이드 4] ✅ 연체는 1일도 하지 마세요
              (연체 기록 = 최대 -100점)

[슬라이드 5] ✅ 오래된 카드는 해지하지 마세요
              (거래기간 = 신용점수 플러스)

[슬라이드 6] 💡 지금 바로 무료 한도조회
              👉 saeloan.co.kr
              ☎ 1555-2137

[슬라이드 7] ⚠️ 연 6.9~19.9% | 새론금융대부중개 등록번호 2026-수원-2324
              과도한 빚은 당신에게 큰 불행을 안겨줄 수 있습니다
```

---

### 샘플 2: 카카오채널 메시지 (상품 안내형)
```
📢 [새론금융] 이번 주 대부업체 금리 안내

안녕하세요, 새론금융대부중개 김덕진 대표입니다.

이번 주 연동 대부업체 금리 현황입니다.

💡 내 아파트 담보로 얼마나 받을 수 있는지
   무료로 확인해 보세요 (신용점수 영향 없음)

👉 saeloan.co.kr

⚠️ 연이자율 6.9~19.9% | 등록번호 2026-수원-2324
과도한 빚은 당신에게 큰 불행을 안겨줄 수 있습니다
※ 중개수수료 없음 | 대출 실행은 등록 대부업체가 진행
```

---

### 샘플 3: 유튜브 쇼츠 스크립트
> 얼굴 출연 없이 자막+화면 구성만으로 제작. 나레이션은 TTS 또는 김덕진 대표 직접 녹음.
```
제목: "대출 거절 당하는 사람들의 공통점 3가지"

[0-3초] (자막+화면) "이거 모르면 또 거절됩니다"

[3-15초] "첫째, 여러 곳에 동시 신청.
          은행은 단기간 다중 조회를 부정적으로 봅니다.
          새론금융에서는 한 번에 6개사를 비교해드립니다."

[15-30초] "둘째, 소득 증빙 서류 누락.
           건강보험료 납부확인서 하나로 해결됩니다."

[30-45초] "셋째, 기존 대출 숨기기.
           솔직하게 말씀해 주셔야 최적의 상품을 찾을 수 있습니다."

[45-55초] "새론금융 무료 한도조회로
           내 조건에 맞는 최저금리를 찾아보세요."

[55-60초] "saeloan.co.kr | 1555-2137
           연 6.9~19.9% | 과도한 빚은 큰 불행을..."
```

---

## 7. 단계별 실행 로드맵

### Phase 1 (1~2주차): 기반 구축
- [ ] 카카오 비즈니스 채널 개설 (등록증 제출)
- [ ] 인스타그램 비즈니스 계정 전환
- [ ] Canva Pro 브랜드 템플릿 제작 (로고, 색상: 남색+노란색)
- [ ] Buffer 계정 연동 (인스타그램 + 카카오)
- [ ] 카카오 챗봇 기본 시나리오 설정

### Phase 2 (3~4주차): 콘텐츠 첫 발행
- [ ] 인스타그램 첫 9장 게시물 (피드 구성)
- [ ] 네이버 블로그 SEO 포스팅 4건
- [ ] 유튜브 채널 개설 + 쇼츠 4편
- [ ] 카카오채널 첫 발송

### Phase 3 (2개월차~): AI 자동화 본격 가동
- [ ] ChatGPT API 연동 콘텐츠 초안 자동생성
- [ ] Google Sheets 리드 자동집계
- [ ] 자동 이메일 팔로업 설정
- [ ] 월간 성과 분석 (도달률, 클릭률, 상담 전환율)

---

## 8. KPI (핵심 성과 지표)

> ⚠️ 아래 목표는 SNS_SALES_SYSTEM.md v4.0 (5-Track 전략) 기준입니다.

| 지표 | 3개월 목표 | 6개월 목표 |
|---|---|---|
| 인스타그램 팔로워 | 300명 | 1,000명 |
| 카카오채널 구독자 | 200명 | 800명 |
| 유튜브 구독자 | 100명 | 500명 |
| 월 홈페이지 유입 | 500건 | 1,500건 |
| 월 상담신청 | 30건 | 80건 |
| **월 실행 건수** | **17건 (누적 50건)** | **25건 (누적 150건)** |

---

## 9. 주의사항 요약

1. **모든 광고물에 반드시 포함:**
   - 등록번호
   - 금리 범위 (연 6.9~19.9%)
   - 경고 문구

2. **절대 하지 말 것:**
   - "100% 승인 가능" 등 허위·과장 표현
   - SNS DM으로 개인정보(주민번호, 계좌) 수집
   - 이메일/폼 주소 SNS 상 직접 노출

3. **SNS→홈페이지 단방향 유도 원칙:**
   - 모든 CTA는 `saeloan.co.kr` 링크로 통일
   - 상담신청, 한도조회 모두 홈페이지에서 처리

---

---

## 10. ⚠️ 운영자 신분 보호 규칙 요약

| 규칙 | 내용 |
|---|---|
| SNS 대외 명의 | 김덕진 대표만 |
| 이희전 이름 노출 | 모든 채널 절대 금지 |
| 얼굴·목소리 출연 | 김덕진 직접 출연 or 출연 없이 제작 |
| 계정 개설 주체 | 없음 (내부 운영자 비공개) |
| 상담 연결 | saeloan.co.kr → 1555-2137 (김덕진 명의) |
| 운영 위임 근거 | 김덕진 대표 구두 승인 완료 |

> 이 구조는 이희전 팀장의 우리은행 소속 대출상담사 겸직 문제를 방지하기 위한 필수 운영 체계입니다.
> 외부에 노출되는 모든 정보는 김덕진 대표 기준으로만 작성·발행합니다.

*대외 명의: 김덕진 대표 | 새론금융대부중개 | 등록번호 2026-수원-2324*
*saeloan.co.kr | 1555-2137 | 최종 수정: 2026-05-21*
